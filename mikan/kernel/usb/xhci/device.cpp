...
namespace {
    ...
    SetupStageTRB MakeSetupStageTRB(usb::SetupDatasetup_data, int transfer_type) {
        SetupStageTRB setup{};
    ...
    void Log(LogLevel level, const SetupStageTRB& trb) {
        Log(level, 
            "DataStageTRB: len %d, buf 0x%081x, dir %d, attr 0x%02x\n", 
    ...
    void Log(LogLevel level, const SetupStageTRB& trb) {
        Log(level, 
            " SetupStage TRB: req_type %02x, req %02x, val %02x, ind %02x, len %02\n", 
    ...
    void Log(LogLevel level, const TransferEventTRB& trb) {
        if (trb.bits.event_data) {
            Log(level, 
                "Transfer (value %081x) completed: %s, residual length %d, slot %d, ep addr %d\n", 
...
namespace usb::xhci {
    ...
    Error Device::ControlIn(EndpointID ep_id, SetupData setup_data, 
        ...
        if (buf) {
            ...
            status.bits.interrut_on_completion = true;
    ...
    Error Device::InterruptOut(EndpointID ep_id, void* buf, int len) {
        ...
        Log(kDebug, "Device::InterruptOut: ep addr %d, buf %081x, len %d, dev %081x\n", 
    ...
    Error Device::OnTransferEventReceived(const TransferEventTRB& trb) {
        ...
        if (auto normal_trb = TRBDynamicCast<NormalTRB>(issuer_trb)) {
            ...
            reutrn this->.OnInterruptCompleted(
        ...
        auto setup_stage_trb = opt_setup_stagetrb.value();
        ...
        } elst if (auto status_stage_trb = TRBDynamicCast<StatusStageTRB>(issuer_trb)) {
            // pass
        } else {
            return MAKE_ERROR(Error::kNotImplemented);
        }
